...
namespace  SingularityEngine::SERenderer
{
	...
	{
	public:
		VulkanImage(std::weak_ptr<VulkanDevice> device, VkImageType type, VkFormat format, VkExtent3D size, uint32_t numMipmaps, uint32_t layers, VkSampleCountFlagBits samples, VkImageUsageFlags usageScenarios, bool isCubemap);
		VulkanImage(std::weak_ptr<VulkanDevice> device, VkImageType type, VkFormat format, VkExtent3D size, uint32_t numMipmaps, uint32_t layers, VkSampleCountFlagBits samples, VkImageTiling tiling, VkImageUsageFlags usageScenarios, bool isCubemap);
