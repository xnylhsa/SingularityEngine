...
namespace SingularityEngine::SERenderer
{
	...
	bool VulkanSwapChain::initialize()
	{
		...
		ASSERT(vulkanDevice != nullptr, "[SERenderer::VulkanSwapChain] device was lost before inialization");
		if (!vulkanDevice) return false;

		mAllocator = std::make_unique<VulkanMemoryAllocator>(vulkanDevice, "SwapChain");
		createSemaphores();
		VkResult result = Platform::CreateSurface(&mPresentationSurface);
		...
		vkGetPhysicalDeviceQueueFamilyProperties(*vulkanDevice.get(), &queueFamiliesCount, nullptr);
		if (queueFamiliesCount == 0)
		{
			ASSERT(false, "Physical device %p does not have any queue families.", vulkanDevice.get());
			return 0;
		}
		std::vector<VkQueueFamilyProperties> queueFamilyProperties(queueFamiliesCount);
		vkGetPhysicalDeviceQueueFamilyProperties(*vulkanDevice.get(), &queueFamiliesCount, queueFamilyProperties.data());
		vkGetPhysicalDeviceQueueFamilyProperties(*vulkanDevice.get(), &queueFamiliesCount, nullptr);
		if (queueFamiliesCount == 0)
		{
			ASSERT(false, "Physical device %p does not have any queue families.", vulkanDevice.get());
		...
		for (size_t i = 0; i < queueFamilyProperties.size(); i++)
		{
			...
			vkGetPhysicalDeviceSurfaceSupportKHR(*vulkanDevice.get(), static_cast<uint32_t>(i), mPresentationSurface, &presentSupport);
	...
	{
		auto vulkanDevice = mDevice.lock();
		ASSERT(vulkanDevice != nullptr, "[SERenderer::VulkanSwapChain] device was lost before creation");
		...
		VkResult result = vkCreateSwapchainKHR(vulkanDevice->getLogicalDevice(), &swapchain_create_info, nullptr, &mSwapChain);
	...
	{
		auto vulkanDevice = mDevice.lock();
		ASSERT(vulkanDevice != nullptr, "[SERenderer::VulkanSwapChain] device was lost before destruction");
		...
		if (mSwapChain)
		{
			...
			{
				vkDestroyImageView(vulkanDevice->getLogicalDevice(), mSwapchainBuffer[i].imageView, nullptr);
			}
			vkDestroySwapchainKHR(vulkanDevice->getLogicalDevice(), mSwapChain, nullptr);
			mDevice.reset();
	...
	bool VulkanSwapChain::checkAvailablePresentationModes(std::vector<VkPresentModeKHR>& presentationModes)
	{
		...
		VkResult result = vkGetPhysicalDeviceSurfacePresentModesKHR(vulkanDevice->getPhysicalDevice(), mPresentationSurface, &modeCount, nullptr);
		...
		result = vkGetPhysicalDeviceSurfacePresentModesKHR(vulkanDevice->getPhysicalDevice(), mPresentationSurface, &modeCount, &presentationModes[0]);
		if ((result != VK_SUCCESS) || modeCount == 0u)
		{
	...
	bool VulkanSwapChain::getSurfaceCapabilities(VkSurfaceCapabilitiesKHR& surfaceCapabilities)
	{
		...
		if (vkGetPhysicalDeviceSurfaceCapabilitiesKHR(vulkanDevice->getPhysicalDevice(), mPresentationSurface, &surfaceCapabilities) != VK_SUCCESS)
		{
	...
	bool VulkanSwapChain::checkAvailableSurfaceFormats(std::vector<VkSurfaceFormatKHR>& surfaceFormats)
	{
		...
		if (vkGetPhysicalDeviceSurfaceFormatsKHR(vulkanDevice->getPhysicalDevice(), mPresentationSurface, &formatsCount, nullptr) != VK_SUCCESS || 0 == formatsCount)
		{
			ASSERT(false, "[Graphics::Swapchain] Could not get the number of supported surface formats.");
			return false;
		}
		surfaceFormats.resize(formatsCount);
		if (vkGetPhysicalDeviceSurfaceFormatsKHR(vulkanDevice->getPhysicalDevice(), mPresentationSurface, &formatsCount, surfaceFormats.data()) != VK_SUCCESS || 0 == formatsCount)
		{
	...
	VkResult VulkanSwapChain::acquireNextSwapchainImage()
	{
		auto vulkanDevice = mDevice.lock();
		ASSERT(vulkanDevice != nullptr, "[SERenderer::VulkanSwapChain] device expired!");
		if (!vulkanDevice) return VK_ERROR_DEVICE_LOST;

		return vkAcquireNextImageKHR(vulkanDevice->getLogicalDevice(), mSwapChain, 2000000000, *mIsPresentComplete, VK_NULL_HANDLE, &mCurrentFrameIndex);
	}
	...
	bool VulkanSwapChain::acquireSwapchainImages()
	{
		...
		result = vkGetSwapchainImagesKHR(vulkanDevice->getLogicalDevice(), mSwapChain, &mNumImages.value(), nullptr);
		...
		result = vkGetSwapchainImagesKHR(vulkanDevice->getLogicalDevice(), mSwapChain, &mNumImages.value(), &mSwapChainImages[0]);
		if ((VK_SUCCESS != result) || (0u == mNumImages))
		{
		...
		for (uint32_t i = 0; i < mNumImages; i++)
		{
			...
			vkCreateImageView(vulkanDevice->getLogicalDevice(), &colorAttachmentView, nullptr, &mSwapchainBuffer[i].imageView);
		}
	...
	void VulkanSwapChain::present()
	{
		...
		if (vkQueueSubmit(vulkanDevice->getGraphicsQueue(), 1, &mSubmitInfo.value(), mWaitFences[mCurrentFrameIndex]) != VK_SUCCESS) return;
		if (!queuePresent(vulkanDevice->getGraphicsQueue(), mCurrentFrameIndex, { *mIsRenderComplete })) return;
		std::vector<VkFence> waitFences(mWaitFences.size());
		for (size_t i = 0; i < waitFences.size(); ++i)
		{
			waitFences[i] = mWaitFences[i];
		}
		VulkanFence::waitForFences(vulkanDevice, waitFences, true, 100000000000);
	}
	...
	{
		mDepthStencil = std::make_unique<VulkanDepthStencil>(mDevice, this, mAllocator.get());
	...
	bool VulkanSwapChain::createFrameBuffer()
	{
		...
		for (uint32_t i = 0; i < numImages; i++)
		{
			...
			mFrameBuffers[i].create(vulkanDevice, *mRenderPass, ivAttachments, width, height, 1);
		}
		return true;
	}
	...
	bool VulkanSwapChain::createRenderPass()
	{
		...
		mRenderPass = std::make_unique<VulkanRenderPass>(vulkanDevice, attachments, subpassDescriptions, dependencies);
	...
	bool VulkanSwapChain::createSemaphores()
	{
		...
		ASSERT(vulkanDevice != nullptr, "[SERenderer::VulkanSwapChain] device expired!");
		if (!vulkanDevice) return false;
		mIsRenderComplete = std::make_unique<VulkanSemaphore>(vulkanDevice, "is render complete");
		mIsPresentComplete = std::make_unique<VulkanSemaphore>(vulkanDevice, "is present complete");
	...
	bool VulkanSwapChain::createFences()
	{
		...
		{
			mWaitFences[i].create(vulkanDevice, true);
		}
		return true;
	}
	...
	void VulkanSwapChain::beginScreenFrame()
	{
		mDrawCommandBuffers.beginRecording(mCurrentFrameIndex, 0, nullptr);
		std::vector<VkClearValue> clearValues(2);
		clearValues[0].color = { {mScreenClearColor.r, mScreenClearColor.g, mScreenClearColor.b, mScreenClearColor.a} };
		...
		VkViewport viewport = {};
		//viewport.y = static_cast<float>(mSurfaceExtent->height);
	...
	void VulkanSwapChain::endScreenFrame()
	{
	...
	void VulkanSwapChain::prepareFrame()
	{
		...
		VulkanFence::waitForFences(vulkanDevice, { mWaitFences[mCurrentFrameIndex] }, true, UINT64_MAX);
		VulkanFence::resetFences(vulkanDevice, { mWaitFences[mCurrentFrameIndex] });
	}
